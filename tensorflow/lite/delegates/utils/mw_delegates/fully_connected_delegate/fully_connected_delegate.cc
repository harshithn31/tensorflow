...
class FullyConnectedDelegateKernel : public SimpleDelegateKernelInterface {
 public:
  ...
  TfLiteStatus Init(TfLiteContext* context,
    ...
    MY_DEBUG_LOG("[DELEGATE-DEBUG: Init] ========In FullyConnectedDelegateKernel::Init===========\n");
    ...
    for(int i = 0; i < params->nodes_to_replace->size; ++i) {
      ...
      MY_DEBUG_LOG("[DELEGATE-DEBUG: Init] Node %d Input_Tidx=%d; Weights_Tidx=%d; Bias_Tidx=%d; output_Tidx=%d\n",
             i, inputs_[i][0], weights_[i][0], biases_[i][0], outputs_[i][0]);
    }
    ...
    MY_DEBUG_LOG("[DELEGATE-DEBUG: Init] FullyConnectedDelegateKernel::Init completed successfully\n\n\n");
    return kTfLiteOk;
  }
  ...
  TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) override {
    ...
    MY_DEBUG_LOG("[DELEGATE-DEBUG: Prepare] In FullyConnectedDelegateKernel::Prepare\n");
    int node_count = inputs_.size();
    int out_tid = 0;
    TF_LITE_KERNEL_LOG(context, "Preparing %d nodes in partition\n\n", node_count);
    MY_DEBUG_LOG("[DELEGATE-DEBUG: Prepare] Preparing %d nodes in partition\n", node_count);
    for(int i = 0; i < node_count; i++){
      ...
      MY_DEBUG_LOG("[DELEGATE-DEBUG: Prepare] Processing node %d with Input_Tidx=%d, Weights_Tidx=%d, Output_Tidx=%d\n",
             i, inputs_[i][0], weights_[i][0], outputs_[i][0]);
      ...
      if (node_has_bias) {
        ...
        MY_DEBUG_LOG("[DELEGATE-DEBUG: Prepare] Node %d has bias tensor at index %d\n", i, biases_[i][0]);
      }
      ...
      MY_DEBUG_LOG("[DELEGATE-DEBUG: Prepare] Node %d - Output tensor allocation: data=%p, bytes=%d, allocation_type=%d\n", 
             i, output_tensor->data.data, output_tensor->bytes, output_tensor->allocation_type);
      ...
      if (req_temp_out) {
        ...
        if (node_has_bias && bias_tensor){
          ...
          MY_DEBUG_LOG("[DELEGATE-DEBUG: Prepare] Node %d - Input_Shape: [%d, %d], Weights_Shape: [%d, %d], Bias_Shape: [%d], Output_Shape: [%d, %d]\n\n",
                 i, input_tensor->dims->data[0], input_tensor->dims->data[1],
                 weights_tensor->dims->data[0], weights_tensor->dims->data[1],
                 bias_tensor->dims->data[0],
                 output_tensor->dims->data[0], output_tensor->dims->data[1]);
          ...
          TF_LITE_KERNEL_LOG(context, "Node %d - Input_Shape: [%d, %d], Weights_Shape: [%d, %d], Output_Shape: [%d, %d]\n\n",
                             ...
                             weights_tensor->dims->data[0], weights_tensor->dims->data[1],
                             output_tensor->dims->data[0], output_tensor->dims->data[1]);
          MY_DEBUG_LOG("[DELEGATE-DEBUG: Prepare] Node %d - Input_Shape: [%d, %d], Weights_Shape: [%d, %d], Output_Shape: [%d, %d]\n\n",
                 i, input_tensor->dims->data[0], input_tensor->dims->data[1],
                 weights_tensor->dims->data[0], weights_tensor->dims->data[1],
                 output_tensor->dims->data[0], output_tensor->dims->data[1]);
        }
    ...
    MY_DEBUG_LOG("[DELEGATE-DEBUG: Prepare] ========FullyConnectedDelegateKernel::Prepare completed successfully========\n\n\n");
    return kTfLiteOk;
  }

  TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) override {
    TF_LITE_KERNEL_LOG(context, "======== In FullyConnectedDelegateKernel::Eval =========\n");
    MY_DEBUG_LOG("[DELEGATE-DEBUG: Eval] =============In FullyConnectedDelegateKernel::Eval=================\n");
...
class FullyConnectedDelegate : public SimpleDelegateInterface {
 public:
  ...
  bool IsNodeSupportedByDelegate(const TfLiteRegistration* registration,
    ...
    MY_DEBUG_LOG("[DELEGATE-DEBUG] ======= In FullyConnectedDelegate: IsNodeSupportedByDelegate ========\n");
    ...
    MY_DEBUG_LOG("[DELEGATE-DEBUG] IsNodeSupportedByDelegate: Node's builtin_code: %d\n",
                registration->builtin_code);
    ...
    MY_DEBUG_LOG("[DELEGATE-DEBUG] ======= FullyConnectedDelegate: Node support check complete ========\n");
    return true;
  }
