...
class FullyConnectedDelegateKernel : public SimpleDelegateKernelInterface {
 public:
  ...
  TfLiteStatus Init(TfLiteContext* context,
    ...
    DEBUG_LOG("[DELEGATE-DEBUG: Init] ========In FullyConnectedDelegateKernel::Init===========\n");
    ...
    for(int i = 0; i < params->nodes_to_replace->size; ++i) {
      ...
      DEBUG_LOG("[DELEGATE-DEBUG: Init] Node %d Input_Tidx=%d; Weights_Tidx=%d; Bias_Tidx=%d; output_Tidx=%d\n",
             i, inputs_[i][0], weights_[i][0], biases_[i][0], outputs_[i][0]);
    }
    ...
    DEBUG_LOG("[DELEGATE-DEBUG: Init] FullyConnectedDelegateKernel::Init completed successfully\n\n\n");
    return kTfLiteOk;
  }
  ...
  TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) override {
    ...
    DEBUG_LOG("[DELEGATE-DEBUG: Prepare] In FullyConnectedDelegateKernel::Prepare\n");
    int node_count = inputs_.size();
    int out_tid = 0;
    TF_LITE_KERNEL_LOG(context, "Preparing %d nodes in partition\n\n", node_count);
    DEBUG_LOG("[DELEGATE-DEBUG: Prepare] Preparing %d nodes in partition\n", node_count);
    for(int i = 0; i < node_count; i++){
      ...
      DEBUG_LOG("[DELEGATE-DEBUG: Prepare] Processing node %d with Input_Tidx=%d, Weights_Tidx=%d, Output_Tidx=%d\n",
             i, inputs_[i][0], weights_[i][0], outputs_[i][0]);
      ...
      if (node_has_bias) {
        ...
        DEBUG_LOG("[DELEGATE-DEBUG: Prepare] Node %d has bias tensor at index %d\n", i, biases_[i][0]);
      }
      ...
      DEBUG_LOG("[DELEGATE-DEBUG: Prepare] Node %d - Output tensor allocation: data=%p, bytes=%d, allocation_type=%d\n", 
             i, output_tensor->data.data, output_tensor->bytes, output_tensor->allocation_type);
      ...
      if (req_temp_out) {
        ...
        if (node_has_bias && bias_tensor){
          ...
          DEBUG_LOG("[DELEGATE-DEBUG: Prepare] Node %d - Input_Shape: [%d, %d], Weights_Shape: [%d, %d], Bias_Shape: [%d], Output_Shape: [%d, %d]\n\n",
                 i, input_tensor->dims->data[0], input_tensor->dims->data[1],
                 weights_tensor->dims->data[0], weights_tensor->dims->data[1],
                 bias_tensor->dims->data[0],
                 output_tensor->dims->data[0], output_tensor->dims->data[1]);
          ...
          TF_LITE_KERNEL_LOG(context, "Node %d - Input_Shape: [%d, %d], Weights_Shape: [%d, %d], Output_Shape: [%d, %d]\n\n",
                             ...
                             weights_tensor->dims->data[0], weights_tensor->dims->data[1],
                             output_tensor->dims->data[0], output_tensor->dims->data[1]);
          DEBUG_LOG("[DELEGATE-DEBUG: Prepare] Node %d - Input_Shape: [%d, %d], Weights_Shape: [%d, %d], Output_Shape: [%d, %d]\n\n",
                 i, input_tensor->dims->data[0], input_tensor->dims->data[1],
                 weights_tensor->dims->data[0], weights_tensor->dims->data[1],
                 output_tensor->dims->data[0], output_tensor->dims->data[1]);
        }
    ...
    DEBUG_LOG("[DELEGATE-DEBUG: Prepare] ========FullyConnectedDelegateKernel::Prepare completed successfully========\n\n\n");
    return kTfLiteOk;
  }

  TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) override {
    TF_LITE_KERNEL_LOG(context, "======== In FullyConnectedDelegateKernel::Eval =========\n");
    DEBUG_LOG("[DELEGATE-DEBUG: Eval] =============In FullyConnectedDelegateKernel::Eval=================\n");
    TF_LITE_KERNEL_LOG(context, "Evaluating %d nodes in partition\n", inputs_.size());
    DEBUG_LOG("[DELEGATE-DEBUG: Eval] Evaluating %d nodes in partition\n", inputs_.size());
    ...
    for(int i = 0; i < inputs_.size(); i++) {
      ...
      DEBUG_LOG("[DELEGATE-DEBUG: Eval] Evaluating node %d with input_Tidx=%d, output_Tidx=%d\n",
             i, inputs_[i][0], outputs_[i][0]);
      const TfLiteTensor& input_tensor = context->tensors[inputs_[i][0]];
      TfLiteTensor& output_tensor = context->tensors[outputs_[i][0]];
      ...
      DEBUG_LOG("[DELEGATE-DEBUG: Eval] Node %d - Tensor sizes - Input: %d, Output: %d\n", i, input_size, output_size);
      DEBUG_LOG("[DELEGATE-DEBUG: Eval] Node %d - Input tensor shape: [%d, %d]\n", 
             i, input_tensor.dims->data[0], input_tensor.dims->data[1]);
      DEBUG_LOG("[DELEGATE-DEBUG: Eval] Node %d - Output tensor shape: [%d, %d]\n", 
             i, output_tensor.dims->data[0], output_tensor.dims->data[1]);
      ...
      if (input_tensor.type == kTfLiteFloat32) {
        ...
        DEBUG_LOG("[DELEGATE-DEBUG: Eval] Node %d: input_tensor.data.f address: %p, input_features: %d\n", 
        ...
        DEBUG_LOG("[DELEGATE-DEBUG: Eval] Node %d - Calling write_input_to_bram with input_features: %d\n", i, input_features);
      ...
      DEBUG_LOG("[DELEGATE-DEBUG: Eval] Node %d - Calling fpga_compute with input_features: %d, output_features: %d\n", i, input_features, output_features);
      fflush(stdout);
      ...
      if (output_tensor.type == kTfLiteFloat32) {
        ...
        DEBUG_LOG("[DELEGATE-DEBUG: Eval] Node %d: output_tensor.data.f address: %p, output_features: %d\n", 
    ...
    DEBUG_LOG("[DELEGATE-DEBUG: Eval] =============FullyConnectedDelegateKernel::Eval completed successfully=================\n");
    return kTfLiteOk;
  }
...
class FullyConnectedDelegate : public SimpleDelegateInterface {
 public:
  ...
  bool IsNodeSupportedByDelegate(const TfLiteRegistration* registration,
    ...
    DEBUG_LOG("[DELEGATE-DEBUG] ======= In FullyConnectedDelegate: IsNodeSupportedByDelegate ========\n");
    ...
    DEBUG_LOG("[DELEGATE-DEBUG] IsNodeSupportedByDelegate: Node's builtin_code: %d\n",
                registration->builtin_code);
    ...
    DEBUG_LOG("[DELEGATE-DEBUG] ======= FullyConnectedDelegate: Node support check complete ========\n");
    return true;
  }
